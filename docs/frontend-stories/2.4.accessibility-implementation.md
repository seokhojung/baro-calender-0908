...
- **Technical KPI**:
  ...
  - 터치 타곃 최소 44px
  - jest-axe 테스트 100% 통과
- **Definition of Success**: WCAG 2.2 AA 완전 준수, 자동 테스트 통과

## Story
**As a** 장애가 있는 사용자,
**I want** 스크린 리더, 키보드, 고대비 모드 등으로 완전히 접근 가능한 캘린더를 사용하여,
**so that** 누구나 차별 없이 편리하게 일정을 관리할 수 있다.
...
**사용자로서, 나는:**
1. 스크린 리더를 사용해도 모든 기능에 접근하고 싶다
2. 키보드만으로도 완전히 앱을 조작하고 싶다
3. 시각 장애가 있어도 충분한 색상 대비로 내용을 읽고 싶다
4. 운동 장애가 있어도 충분한 시간을 갖고 조작하고 싶다
5. 다양한 언어와 문화권에서 자연스럽게 사용하고 싶다
...
export const useScreenReader = () => {
  ...
  useEffect(() => {
    ...
    const detectScreenReader = () => {
      ...
      // 3. prefers-reduced-motion 확인 (간접 지표)
...
export class ColorContrastManager {
  // WCAG AA 기준: 4.5:1 (일반 텍스트), 3:1 (대형 텍스트)
  // WCAG AAA 기준: 7:1 (일반 텍스트), 4.5:1 (대형 텍스트)
...
export const AccessibilitySettings: React.FC = () => {
  ...
  return (
    <div className="accessibility-settings space-y-6">
      <h2 className="text-lg font-semibold">접근성 설정</h2>
  ...
  return (
    <div className="border rounded p-4" style={{ backgroundColor: bgColor }}>
      <p style={{ color: textColor }} className="mb-2">
        일반 텍스트 (대비: {contrast1.toFixed(1)}:1)
      </p>
      <p style={{ color: primaryColor }} className="font-medium">
        강조 텍스트 (대비: {contrast2.toFixed(1)}:1)
      </p>
    </div>
  )
}
...
const KeyboardShortcuts: React.FC = () => {
  const shortcuts = [
    ...
    { key: '화살표 키', description: '캘린더 날짜 네비게이션' },
    { key: 'Enter / Space', description: '선택 또는 활성화' },
    { key: 'Escape', description: '모달 닫기 또는 취소' },
    { key: 'Home / End', description: '주의 시작/끝으로 이동' },
    { key: 'Page Up/Down', description: '월 네비게이션' },
  ]
...
export const testKeyboardNavigation = (container: HTMLElement) => {
  ...
  // 모든 focusable 요소가 키보드로 접근 가능한지 확인
...
**이 스토리는 바로캘린더의 접근성 구현을 위한 종합적인 기술 가이드입니다. 모든 사용자가 장애 여부에 관계없이 동등하게 서비스를 이용할 수 있도록 포용적인 사용자 경험을 제공합니다.**